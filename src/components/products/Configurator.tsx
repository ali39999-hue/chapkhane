...
/**
 * Lean projections built by the server page. Keeping these explicit (rather
 * than `any`) is what keeps `seoContent` richText and populated relationship
 * arrays out of the client bundle.
 */
...
export type ConfiguratorPaper = {
  id: number | string;
  name: string;
  allowedGrammages?: Array<{ grammage: number }> | null;
};
...
type ConfiguratorProps = {
  ...
  papers: ConfiguratorPaper[];
  sizes: ConfiguratorSize[];
  finishings: ConfiguratorOption[];
  turnarounds: ConfiguratorOption[];
};

const DEFAULT_GRAMMAGES = [{ grammage: 80 }, { grammage: 100 }, { grammage: 120 }];
...
export function Configurator({ product, papers, sizes, finishings, turnarounds }: ConfiguratorProps) {
  ...
  const [selectedTurnaround, setSelectedTurnaround] = useState(String(turnarounds[0]?.id ?? ""));
  ...
  const activePaper = papers.find(p => String(p.id) === selectedPaper);
  const activeSize = sizes.find(s => String(s.id) === selectedSize);
  const activeFinishing = finishings.find(f => String(f.id) === selectedFinishing);
  const activeTurnaround = turnarounds.find(t => String(t.id) === selectedTurnaround);
  ...
  const config = useMemo(
    () => ({
      ...
      size: {
        id: selectedSize,
        width: activeSize?.finalWidth ?? undefined,
        height: activeSize?.finalHeight ?? undefined,
      },
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // 422 = the combination has no tariff row. Surface the server's
          // Persian explanation instead of leaving a stale price on screen.
          setQuote(null);
          setQuoteError(
            typeof data?.error === "string"
              ? data.error
              : "برای این ترکیب، قیمتی ثبت نشده است. لطفاً گزینه دیگری انتخاب کنید."
          );
        }
      } catch {
  ...
  }, [config, product.slug, selectedPaper, selectedTurnaround]);

  const totalPrice = quote?.total ?? null;

  /**
   * Changing the paper also resets the grammage to the first one that paper
   * allows. Doing it here rather than in an effect avoids a cascading render
   * (and a request for a config that was never selectable).
   */
  const handlePaperChange = (paperId: string) => {
    setSelectedPaper(paperId);
    const paper = papers.find((p) => String(p.id) === paperId);
    const firstGrammage = paper?.allowedGrammages?.[0]?.grammage;
    if (firstGrammage !== undefined) setGrammage(firstGrammage);
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 items-start font-sans">
      ...
      <div className="w-full lg:w-2/3 flex flex-col gap-6">
        ...
        <div className="bg-white border border-secondary-200 rounded-2xl p-6 sm:p-8 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              <label htmlFor="cfg-paper" className="text-xs font-bold text-secondary-600 flex justify-between">
                نوع متریال (کاغذ)
              </label>
              <div className="relative">
                ...
                <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-secondary-400 pointer-events-none" aria-hidden="true" />
              </div>
            </div>

            {/* Grammage */}
            <div className="space-y-3">
              <label htmlFor="cfg-grammage" className="text-xs font-bold text-secondary-600 flex justify-between">
              ...
              <div className="relative">
                <select 
                  ...
                  className="w-full h-12 pl-4 pr-10 rounded-xl bg-secondary-50 border border-secondary-200 text-sm font-bold text-secondary-900 appearance-none focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none cursor-pointer"
                >
                  {(activePaper?.allowedGrammages?.length ? activePaper.allowedGrammages : DEFAULT_GRAMMAGES).map((g, idx) => (
            ...
            {/* Size Selection */}
            <div className="space-y-3">
              <label htmlFor="cfg-size" className="text-xs font-bold text-secondary-600 flex justify-between">
            ...
            {/* Finishing Selection */}
            <div className="space-y-3">
              <label htmlFor="cfg-finishing" className="text-xs font-bold text-secondary-600 flex justify-between">
            ...
            <div className="space-y-3">
              <label htmlFor="cfg-turnaround" className="text-xs font-bold text-secondary-600 flex justify-between">
                زمان تحویل (لجستیک)
              </label>
              <div className="relative">
                <select 
            ...
            {/* Quantity — a radio group, so arrow keys move between options and
                the current choice is announced. */}
            <fieldset className="space-y-3">
              <legend className="text-xs font-bold text-secondary-600 mb-3">تیراژ چاپ (عدد)</legend>
        ...
        <div
          className="bg-white border border-secondary-200 rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center text-center relative overflow-hidden group"
          aria-live="polite"
        >
          <div className="absolute inset-0 bg-grid-dots opacity-20"></div>
          
          <label htmlFor="artwork-file" className="sr-only">
            انتخاب فایل طراحی برای بررسی Preflight
          </label>
          <input
            id="artwork-file"
            ref={fileInputRef}
      ...
      <div className="w-full lg:w-1/3 sticky top-24">
        <div className="bg-white border border-secondary-200 rounded-2xl shadow-float overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            ...
            {quoteError && (
              <div
                className="p-3 bg-red-50 border border-red-200 text-red-700 text-xs rounded-lg font-bold flex items-start gap-2"
                role="status"
              >
            ...
            {quote?.warnings && quote.warnings.length > 0 && (
              <div className="p-3 bg-amber-50 border border-amber-200 text-amber-700 text-xs rounded-lg font-medium space-y-1">
                {quote.warnings.map((w, idx) => (
                  <p key={idx} className="flex items-center gap-1"><HelpCircle size={12} aria-hidden="true" /> {w}</p>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="p-6 bg-secondary-900 text-white">
            ...
            {submitError && (
              <p className="text-center text-xs text-red-300 mt-3 font-bold" role="alert">{submitError}</p>
            )}
